...
- **Strict Scope Boundary**: Edits must modify exclusively the files declared in the task prompt. Modifying unlisted files constitutes scope creep and fails review immediately.

## 2. Verifiable Completion Criteria

- **Deterministic Verification**: Every completion criterion must be an observable check: a file's existence in a specific directory or an automated command exiting with code 0.
- **Deterministic Boundaries**: Express milestone boundaries exclusively through observable artifact existence or command execution. Subjective phrasing fails review.
...
- **Scratch Containment**: Store all temporary logs, diff dumps, and test outputs in `.scratch/` or `$env:TEMP`. Root-level scratch dumps fail review immediately.
